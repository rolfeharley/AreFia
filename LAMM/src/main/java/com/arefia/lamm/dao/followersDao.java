...
public interface followersDao extends JpaRepository<followersEntity, Long> {
    ...
    @Query(value = "SELECT MSGFROM, MSGTYPE, CASE WHEN MSGTYPE = 'text' THEN REPLACE(MSG, '\n', '<br/>') ELSE MSG END MSG, FILENAME, " + 
    		"CAST(EXETIME AS CHAR(100)) AS EXETIME, MSGID FROM (SELECT SOURCERID, MSGTYPE, MSG, FILENAME, PUSHTIME AS EXETIME, 'PUSH' AS MSGFROM, " + 
    		"MSGID, CHECKFLAG FROM MSG_PUSH UNION SELECT SOURCERID, MSGTYPE, MSG, '' AS FILENAME, RECIEVETIME AS EXETIME, " + 
    ...
    @Query(value = "SELECT MSGFROM, MSGTYPE, CASE WHEN MSGTYPE = 'text' THEN REPLACE(MSG, '\\n', '<br/>') ELSE MSG END MSG, " + 
    		"FILENAME, CAST(EXETIME AS CHAR(100)) AS EXETIME, MSGID FROM " + 
    		"(SELECT SOURCERID, MSGTYPE, MSG, FILENAME, PUSHTIME AS EXETIME, 'PUSH' AS MSGFROM, MSGID, CHECKFLAG FROM MSG_PUSH " + 
    		"UNION SELECT SOURCERID, MSGTYPE, MSG, '' AS FILENAME, RECIEVETIME AS EXETIME, " + 
