...
public interface menuDao extends JpaRepository<menuEntity, Long> {
	@Query(value = "SELECT A.APPID, A.APPNAME, A.APPURL FROM APP_LIST A " +
                   ...
                   "(SELECT COUNT(*) FROM USER_IN_GROUP WHERE ACCOUNTID = :ACCOUNTID AND GROUPID LIKE 'adm%') > 0) " +
                   "AND C.ISENABLE = '1' " +
                   "GROUP BY A.APPID, A.APPNAME, A.APPURL ORDER BY A.ORDERINGROUP", nativeQuery = true)
